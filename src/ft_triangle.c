...
t_vec3	ft_tri_norm(t_obj tri)
{
	...
	return (ftvec3_normalize(ftvec3_cross(ftvec3_minus(tri.props.rotation,
					tri.props.position),
				ftvec3_minus(v2, tri.props.position))));
}
...
int	detect_hit(t_ray *ray, struct s_tri_val *s, t_vec3 edge[2], t_vec3 v[3])
{
				...
				t_hitrec *rec, struct s_lpair limit)
{
	...
	v[2] = (t_vec3){triangle.props.radius,
		triangle.props.diameter, triangle.props.height};
	edge[0] = ftvec3_minus(v[1], v[0]);
	edge[1] = ftvec3_minus(v[2], v[0]);
	if (detect_hit(&ray, &s, edge, v) == false)
		return (false);
	...
	ft_hitr_set_face_normal(rec, ray,
		ftvec3_normalize(ftvec3_cross(edge[0], edge[1])));
